...
{
	struct kasumi_allow_uid_entry *entry;
	bool found = false;
	...
	hash_for_each_possible_rcu(kasumi_allow_uids_ht, entry, node, uid) {
		if (entry->uid == uid) {
			found = true;
			break;
		}
...
static void kasumi_clear_allowlist_cache(void)
{
	...
	synchronize_rcu();
	hash_for_each_safe(kasumi_allow_uids_ht, bkt, tmp, entry, node) {
...
static void kasumi_add_allow_uid(uid_t uid)
{
	...
	if (!entry)
		return;
	entry->uid = uid;
	hash_add_rcu(kasumi_allow_uids_ht, &entry->node, uid);
}

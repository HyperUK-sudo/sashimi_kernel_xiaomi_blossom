...
DEFINE_HASHTABLE(kasumi_allow_uids_ht, KASUMI_HASH_BITS);
...
void kasumi_cleanup_locked(void)
{
	...
	struct kasumi_allow_uid_entry *allow_uid_entry;
	...
	hash_for_each_safe(kasumi_allow_uids_ht, bkt, tmp, allow_uid_entry, node) {
		hash_del_rcu(&allow_uid_entry->node);
		kfree_rcu(allow_uid_entry, rcu);
	}
